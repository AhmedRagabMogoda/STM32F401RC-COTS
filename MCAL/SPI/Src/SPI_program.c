...
uint8_t SPI_u8Init(SPI_Config_t *Copy_SPIConfig)
{
	...
	if(Copy_SPIConfig!=NULL)
	{
		...
		SPI[Copy_SPIConfig->Number]->CR1 |= (Copy_SPIConfig->CRCEN << DFF_PIN_ACCESS);
...
uint8_t SPI_u8SendData(SPI_Config_t *Copy_SPIConfig,uint16_t *Data,uint32_t DataLength)
{
	uint8_t Local_u8ErrorState = OK;
		if(Copy_SPIConfig!=NULL)
		{
			...
			for(Count=CLEAR;Count<DataLength;Count++)
			{
				...
				while( !((SPI[Copy_SPIConfig->Number]->SR >> TXE_PIN_ACCESS) & ONE) && --Time);
				if(Time == TIME_FINISHED)
				{
					Local_u8ErrorState=TIME_OUT;
				}
				SPI[Copy_SPIConfig->Number]->DR = Data[Count];
			}
		}
		else
		{
...
uint8_t SPI_u8ReceiveData(SPI_Config_t *Copy_SPIConfig,uint16_t *Data,uint32_t DataLength)
{
	uint8_t Local_u8ErrorState = OK;
		if(Copy_SPIConfig!=NULL)
		{
			...
			for(Count=CLEAR;Count<DataLength;Count++)
			{
				...
				while( !((SPI[Copy_SPIConfig->Number]->SR >> RXNE_PIN_ACCESS) & ONE) && --Time);
				if(Time == TIME_FINISHED)
				{
					Local_u8ErrorState=TIME_OUT;
				}
				Data[Count] = SPI[Copy_SPIConfig->Number]->DR;
			}
		}
		else
		{
...
uint8_t SPI_u8SendData_DMA(SPI_Config_t *Copy_SPIConfig)
{
	uint8_t Local_u8ErrorState = OK;
		if(Copy_SPIConfig!=NULL)
		{
			SPI[Copy_SPIConfig->Number]->CR2 |= (TXDMAEN_MASK << TXDMAEN_PIN_ACCESS);
		}
		else
		{
			Local_u8ErrorState = NULL_PTR_ERR;
...
uint8_t SPI_u8ReceiveData_DMA(SPI_Config_t *Copy_SPIConfig)
{
	uint8_t Local_u8ErrorState = OK;
		if(Copy_SPIConfig!=NULL)
		{
			SPI[Copy_SPIConfig->Number]->CR2 |= (RXDMAEN_MASK << RXDMAEN_PIN_ACCESS);
		}
		else
		{
			Local_u8ErrorState = NULL_PTR_ERR;
